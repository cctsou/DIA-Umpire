 ...
 * <p>
 * These come in two flavors: {@code Addition}s and {@code Removal}.
 * {@code Addition}s cover a single index and represents the fact that the
 * containing PatchWorkArray has a sublist at that index in its backingList.
 * {@code Removal}s cover a continuous block of indices in the
 * {@code PatchWorkArray}'s backingList and represent that those indices do not
...
abstract class Alteration implements Comparable<Alteration> {
    ...
    /**
     * Returns the impact on performance which the existence of this Alteration
     * will have on a containing {@code PatchWorkArray}.
     * <p>
     * Let <i>t</i> be the sum of the values returned by calling this method on
     * all {@code Alteration}s in a {@code PatchWorkArray}, the time it takes to
     * find an element in that list by index should be <i>O(log(t))</i>.
     *
    ...
    /**
     * Returns whether or not this {@code Alteration} covers the given index in
     * the {@code PatchWorkArray}'s backingList.
     *
     * @param index the index of thebackingList to check.
     * @return <code>true</code> in the case that this {@code Alteration} covers
     * the given index of the backingList.
     */
    ...
    /**
     * Returns the number of elements that this alteration affects, which is
     * calculated as being the absolute size of the {@code #indexDiff}.
     *
 ...
 * <p>
 * The index of the removal is the smallest index in the block of consecutive
 * removals.
 */
class Removal extends Alteration {
    ...
    /**
     * This should only be called if the element to in position index-1 is a
     * normal element that is due to be removed.
     */
