 ...
 * <p>
 * Performs all the main operations: {@code contains}, {@code add},
 * {@code remove} and {@code get} in time <i>O(log(n))</i>, where <i>n</i> is
 * the number of elements in the list.
 * <p>
 * This implementation is not synchronised so if you need multi-threaded access
 * then consider wrapping it using the {@link Collections#synchronizedList}
 * method.
 * <p>
...
//        stream.writeInt(FinalizedSortedArray.length);
//        for (int i = 0; i < size(); i++) {
//            stream.writeObject(FinalizedSortedArray[i]);
//        }
//    }
//    private void readObject(java.io.ObjectInputStream in) throws ClassNotFoundException, IOException{
//        in.defaultReadObject();
//        Finalized=true;
//        FinalizedSortedArray=new Object[in.readInt()];
//        for (int i = 0; i < size(); i++) {
...
//    }
    ...
    public void ClearTree() {
        if (root != null) {
            Node start = root.smallestNodeInSubTree(); //start with the smallest value.
     ...
     * location, so as to ensure that the elements in the list are kept in the
     * order specified by the given {@code Comparator}.
     * <p>
     * This method only allows non- <code>null</code> values to be added, if the
     * given object is <code>null</code>, the list remains unaltered and false
     * returned.
     *
    ...
    public boolean add(T object) {
        if (Finalized) {
            System.out.print("The array is finalized, insertion is prohibited.");
            return false;
        }
     ...
     * <p>
     * This method can be overridden by a subclass in order to change the
     * definition of the {@code Node}s that this List will store.
     * <p>
     * This implementation uses the {@code Node#compareTo(Node)} method in order
     * to ascertain where the given {@code Node} should be stored. It also
     * increments the modCount for this list.
     *
    ...
    /**
     * Tests if this tree has exactly the same structure and values as the given
     * one, should only be used for testing.
     *
     * @param other the {@code SortedList} to compare this to.
     * @return true if and only if all this {@code SortedList} are structurally
     * the same with each node containing the same values.
     */
    ...
    /**
     * Provides an {@code Iterator} which provides the elements of this
     * {@code SortedList} in the order determined by the given
     * {@code Comparator}.
     * <p>
     * This implementation allows the entire list to be iterated over in time
     * <i>O(n)</i>, where <i>n</i> is the number of elements in the list.
     *
     * @return an {@code Iterator} which provides the elements of this
     * {@code SortedList} in the order determined by the given
     * {@code Comparator}.
     */
    @Override
    public Iterator<T> iterator() {
        if (Finalized) {
            return (Iterator<T>) Arrays.asList(FinalizedSortedArray).iterator();
        }
        return new Itr();
    }
    ...
    public int size() {
        ...
        return FinalizedSortedArray.length;
    }
    ...
    protected Node getRoot() {
        if (Finalized) {
            System.out.print("The array is finalized, operation of tree structure is prohibited.");
            return null;
        }
        return root;
    }
    ...
    /**
     * Returns whether or not the given object is present in this
     * {@code SortedList}. The comparison check uses the
     * {@code Object#equals(Object)} method and work under the assumption that
     * the given <code>obj</code> must have type <code>T</code> to be equal to
     * the elements in this {@code SortedList}. Works in time <i>O(log(n))</i>,
    ...
    /**
     * Returns the node representing the given value in the tree, which can be
     * null if no such node exists.
     * <p>
     * This method performs a binary search using the given comparator, and
     * hence works in time O(log(n)).
     *
     ...
     * {@code SortedList}. Since the list is sorted this is the "index"-th
     * smallest element, counting from 0-<i>n</i>-1.
     * <p>
     * For example, calling {@code remove(0)} will remove the smallest element
     * in the list.
     *
    ...
    public T remove(int index) {
        ...
        T returnNode = nodeAtIndex.value;
        remove(nodeAtIndex);
        return returnNode;
    }

    /**
     * Removes the first element in the list with the given value, if such a
     * node exists, otherwise does nothing. Comparisons on elements are done
    ...
    /**
     * Removes the given {@code Node} from this {@code SortedList}, re-balancing
     * if required, adds to <code>modCount</code> too.
     * <p>
     * Operates in time <i>O(log(n))</i>, where
    ...
    /**
     * Returns the element at the given index in this {@code SortedList}. Since
     * the list is sorted, this is the "index"th smallest element, counting from
     * 0-<i>n</i>-1.
     * <p>
     * For example, calling {@code get(0)} will return the smallest element in
     * the list.
     *
    ...
    public T get(int index) {
        if (Finalized) {
            return (T) FinalizedSortedArray[index];
        }
     ...
     *
     * @return {@code true} if the list has no element in it and {@code false}
     * otherwise.
     */
    @Override
    public boolean isEmpty() {
        ...
        return FinalizedSortedArray.length == 0;
    }
    ...
    /**
     * Returns a new array containing all the elements in this
     * {@code SortedList}.
     *
    ...
    /**
     * Copies the elements in the {@code SortedList} to the given array if it is
     * large enough to store them, otherwise it returns a new array of the same
     * type with the elements in it.
     * <p>
     * If the length of the given array is larger than the size of this
     * {@code SortedList} then, the elements in this list are put into the start
     * of the given array and the rest of the array is left untouched.
     *
     * @return an array representation of this list.
    ...
    /**
     * Returns the smallest balance factor across the entire list, this serves
     * not other purpose other than for testing.
     *
     * @return the minimum of all balance factors for nodes in this tree, or 0
    ...
    /**
     * Returns the largest balance factor across the entire list, this serves
     * not other purpose other than for testing.
     *
     * @return the maximum of all balance factors for nodes in this tree, or 0
     * if this tree is empty.
     */
    ...
    /**
     * Inner class used to represent positions in the tree. Each node stores a
     * list of equal values, is aware of their children and parent nodes, the
     * height of the subtree rooted at that point and the total number of
    ...
    protected class Node implements Comparable<Node>, Serializable {
         ...
         *
         * @return <code>true</code> if this node has two children and
        ...
        /**
         * Returns the grand parent {@code Node} of this {@code Node}, which may
         * be <code>null</code>.
         *
         * @return the grand parent of this node if there is one and
        ...
        /**
         * Returns whether or not this not is the left child of its parent node;
         * if this is the root node, then <code>false</code> is returned.
         *
         * @return <code>true</code> if this is the left child of its parent
        ...
        /**
         * Returns whether or not this not is the right child of its parent
         * node; if this is the root node, then <code>false</code> is returned.
         *
         * @return <code>true</code> if this is the right child of its parent
         * node and <code>false</code> otherwise.
         */
        ...
        /**
         * Returns the left child of this {@code Node}, which may be
         * <code>null</code>.
         *
         * @return the left child of this {@code Node}, which may be
         * <code>null</code>.
         */
        ...
        /**
         * Returns the right child of this {@code Node}, which may be be
         * <code>null</code>.
         *
        ...
        /**
         * Returns the parent {@code Node} of this node, which will be
         * <code>null</code> in the case that this is the root {@code Node}.
         *
        ...
        /**
         * Compares the value stored at this node with the value at the given
         * node using the comparator, if these values are equal it compares the
         * nodes on their IDs; older nodes considered to be smaller.
         *
         * @return if the comparator returns a non-zero number when comparing
         * the values stored at this node and the given node, this number is
         * returned, otherwise this node's id minus the given node's id is
         * returned.
         */
         ...
         *
         * @return the smallest valued node in the tree rooted at this node,
         * which maybe this node.
         */
         ...
         *
         * @return the largest valued node in the tree rooted at this node which
         * may be this node.
         */
        ...
        /**
         * Gets the next biggest node in the tree, which is <code>null</code> if
         * this is the largest valued node.
         *
         * @return the next biggest node in the tree, which is <code>null</code>
         * if this is the largest valued node.
         */
        ...
        /**
         * Gets the node that is the next smallest in the tree, which is
         * <code>null</code> if this is the smallest valued node.
         *
         * @return the node that is the next smallest in the tree, which is
         * <code>null</code> if this is the smallest valued node.
         */
        ...
        /**
         * Returns whether or not this {@code Node} is a leaf; this is true in
         * the case that both its left and right children are set to
         * <code>null</code>.
         *
         * @return <code>true</code> if this node is leaf and <code>false</code>
         * otherwise.
         */
         ...
         *
         * @return a {@code String} representing this {@code Node} for
         * debugging.
         */
        @Override
        ...
        /**
         * Returns, the number of children of this {@code Node} plus one. This
         * method uses a cached variable ensuring it runs in constant time.
         *
        ...
        /**
         * Updates the height and the number of children for nodes on the path
         * to this. Also calls {@code #updateAdditionalCachedValues()}, for
         * every node on the path to this, including this one.
         */
        ...
        /**
         * Called when a node is inserted or removed from the tree and provides
         * a hook for sub-classes to get their cached values updated.
         * <p>
         * This method is called every time the list is altered. It is first
         * called on the deepest node which is affected by a given change and is
         * then subsequently called on ancestors of this node until it is called
         * on the root node.
         * <p>
         * It is therefore only suitable for updating cached values in the case
         * that they are non-global and do not rely on the parent node having
         * the correct value when being calculated.
         * <p>
