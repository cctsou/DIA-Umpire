...
//    }
    ...
    public ArrayList<PeakCluster> FindAllPeakClustersForMappedPep(PepIonID pep) {
        ...
        for (float rt : pep.PredictRT) {
            ...
            float rtrange=Math.min(5, Math.max(pep.GetRTSD()*2,parameter.RTtol));            
...
//        }
    ...
    protected void ExportPeakCurveResultCSV_V3() throws IOException {
        Logger.getRootLogger().info("Writing fragment peak to file:" + FilenameUtils.getFullPath(ScanCollectionName) + FilenameUtils.getBaseName(ScanCollectionName) + "_Frag.csv...");
        FileWriter writer = new FileWriter(FilenameUtils.getFullPath(ScanCollectionName) + FilenameUtils.getBaseName(ScanCollectionName) + "_Frag.csv");
        writer.write("Curve_index,StartRT,EndRT,mz,mzVar,ApexRT,ApexInt\n");
        for (PeakCurve peakCurve : UnSortedPeakCurves) {
            writer.write(peakCurve.Index + "," + peakCurve.StartRT() + "," + peakCurve.EndRT() + "," + peakCurve.TargetMz + "," + peakCurve.MzVar + "," + peakCurve.ApexRT + "," + peakCurve.ApexInt + "\n");
        }
        writer.close();
    }
