...
public class MzXMLthreadUnit implements Runnable {
    ...
    public MzXMLthreadUnit(String XMLtext, InstrumentParameter parameter, SpectralDataType.DataType dataType, UpdateProcess update, boolean ReadPeak) {
    ...
    private void DrawIntDis() {
        ...
        try {
            ChartUtilities.saveChartAsPNG(new File(scan.MsLevel + "_ " + scan.Num + ".PNG"), chart, 1000, 600);
        } catch (IOException e) {
        }
    }
    ...
    public void run() {
        ...
        if (!scan.centroided) {
            scan.Centroiding(parameter.Resolution, parameter.MinMZ);
        }
